...
- **No `AutoName`.** It is generated by `fdroid checkupdates` and used only for
  their bot's commit messages. `fdroid lint` rejects it in a hand-written file,
  and it failed the pipeline on the first submission for exactly this reason.
